// credits to alectrona 9785 :) thank u
...
public class SwerveDriveInput {
    ...
    public Translation2d getShapedInput(DoubleSupplier x, DoubleSupplier y) {
        ...
        // Apply deadband
        if (rawMagnitude < Constants.OperatorConstants.DEADBAND) {
            return new Translation2d(0, 0);
        }

        // Remap magnitude to 0-1 range after deadband
        double remappedMagnitude = Math.min(1.0, (rawMagnitude - Constants.OperatorConstants.DEADBAND) / (1.0 - Constants.OperatorConstants.DEADBAND));
